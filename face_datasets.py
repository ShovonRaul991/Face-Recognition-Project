...
face_id = int(input("Enter the face Id: "))

# Initialize sample face image
count = 0

# Ip of the IP webcam server (on phone). The phone and your computer must be in the same LAN (connected to the same WiFi)
url = 'http://10.159.214.134:8080/shot.jpg'   # it is specific ip adress for Ip Camera inatalled in my phone.
...
while(True):
    ...
    faces = face_detector.detectMultiScale(gray, 1.3, 5)    #scale_factor and min_neighbour as arguments
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite("dataset/User." + str(face_id) + '.' + str(count) + ".jpg", gray[y:y+h,x:x+w])   # for vector format gray is used
